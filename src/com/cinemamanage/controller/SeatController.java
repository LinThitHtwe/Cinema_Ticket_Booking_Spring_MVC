...
public class SeatController {
	...
	public ModelAndView setupUpdatePaymentMethod(@PathVariable String seatId,HttpSession session){
	    if(session.getAttribute("currentEmail")==null) {
	        return new ModelAndView("errorPage","bean",new SeatBean());
	       }
	...
	public String updatePaymentMethod(@ModelAttribute("seatBean") @Validated SeatBean bean, 
		...
		if(Integer.parseInt(bean.getSeatPrice())<999) {
			model.addAttribute("error","Update Failed! Seat Price cannot be lower than 1000MMK");
			return "seatUpdate";
		}
		
		if(!bean.getSeatPrice().endsWith("00")) {
			model.addAttribute("error","Please put the proper price");
			return "seatUpdate";
		}
