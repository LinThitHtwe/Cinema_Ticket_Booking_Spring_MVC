...
public class MovieDAO {
	...
	public int insertData(MovieRequestDTO dto) {
		int result=0;
		String sql="insert into movie(movie_name,movie_createtime,movie_genre,movie_duration,movie_discription,created_person,movie_image) values(?,?,?,?,?,?,?)";
	...
	public int updateData(movieAndTimetableRequestDTO dto) {
		String sql= "UPDATE movie\r\n" + 
				...
				"SET movie.movie_name = ?, movie_updatetime=?,movie.movie_genre=?,movie_duration=?,movie_discription=?,updated_person=?,timetable.start_time = ?,timetable.start_date=?, movie.movie_image=? \r\n" + 
				"WHERE movie.id=?;\r\n" + 
				"";
	...
	public ArrayList<movieAndTimetableResponseDTO> checkingTooClose(String startDate) {
		...
		try {
			...
			while(rs.next()) {
				...
				res.setMovieDescription(rs.getString("movie_discription"));
	...
	public movieAndTimetableResponseDTO selectOneAdmin(MovieRequestDTO dto) {
		movieAndTimetableResponseDTO res = new movieAndTimetableResponseDTO();
		String sql="SELECT *\r\n" + 
				"FROM movie m  \r\n" + 
		...
		try {
			...
			while(rs.next()) {
				...
				res.setMoviePoster(rs.getNString("movie_image"));
			}
