...
public class PaymentMethodDAO {
	...
	public int deleteData(PaymentMethodRequestDTO dto) {
		int result = 0;
		...
		try {
			...
			ps.setInt(2,dto.getPaymentMethodID());
			result = ps.executeUpdate();
		} catch (SQLException e) {
			System.out.println("DataBase Error "+e); 
		}return result;
...
//	
	...
	public ArrayList<PaymentMethodResponseDTO> selectAllNonDeletedPayment() {
		ArrayList<PaymentMethodResponseDTO> list = new ArrayList();
		String sql= "SELECT * FROM payment_method WHERE (status IS NULL OR status = \"\");\r\n" + 
				"";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs= ps.executeQuery();
	...
	public ArrayList<PaymentMethodResponseDTO> selectAllDeletedPayment() {
		ArrayList<PaymentMethodResponseDTO> list = new ArrayList();
		String sql= "select * from payment_method IS NOT NULL";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs= ps.executeQuery();
			while(rs.next()) {
	...
	public boolean isPhoneNoExists(String paymentName,String phoneNo) {
	    ...
	    try {
	        ...
	        String sql = "SELECT * FROM payment_method WHERE payment_method_name=? and payment_method_phone = ? ";
	         ...
	         ps.setString(1,paymentName.toUpperCase());
	...
	public ArrayList<PaymentMethodResponseDTO> searchByPaymentMethodId(PaymentMethodRequestDTO dto) {
		ArrayList<PaymentMethodResponseDTO> list = new ArrayList();
		String sql= "select * from payment_method where id=? and  (status IS NULL OR status = \"\");"  ;
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setInt(1, dto.getPaymentMethodID());
			ResultSet rs= ps.executeQuery();
  ...
  public ArrayList<PaymentMethodResponseDTO> selectByPaymentMethodName(PaymentMethodRequestDTO dto) {
	    ...
	    String sql="SELECT * FROM payment_method WHERE payment_method_name LIKE CONCAT('%',?, '%') and (status IS NULL OR status = \"\");";	
	    try {
	        PreparedStatement ps = con.prepareStatement(sql);
	        ps.setString(1, dto.getPaymentMethodName());
	        ResultSet rs= ps.executeQuery();
  ...
  public ArrayList<PaymentMethodResponseDTO> selectByPaymentMethodPhone(PaymentMethodRequestDTO dto) {
	    ArrayList<PaymentMethodResponseDTO> list = new ArrayList<PaymentMethodResponseDTO>();
	    String sql="SELECT * FROM payment_method WHERE payment_method_phone LIKE CONCAT('%',?, '%') and (status IS NULL OR status = \"\");";	
	    try {
	        PreparedStatement ps = con.prepareStatement(sql);
  ...
  public ArrayList<PaymentMethodResponseDTO> selectByPaymentMethodAll(PaymentMethodRequestDTO dto) {
	    ArrayList<PaymentMethodResponseDTO> list = new ArrayList<PaymentMethodResponseDTO>();
	    String sql="SELECT * FROM payment_method WHERE id=? and payment_method_name LIKE CONCAT('%',?, '%') and payment_method_phone LIKE CONCAT('%',?, '%') and (status IS NULL OR status = \"\");";	
	    try {
	        PreparedStatement ps = con.prepareStatement(sql);
	        ps.setInt(1, dto.getPaymentMethodID());
